...
/**
 * Renderer responsible for displaying action of type {@link WorkflowActionType#END}
 */
public class ActionEndRenderer extends AbstractActionRenderer
{
    public static final String VELOCITY_TEMPLATE_LOCATION = "/assets/shapes/endAction.svg.vm";
    public static final String VELOCITY_IE_TEMPLATE_LOCATION = "/assets/shapes/endAction_ie.svg.vm";


    @Override
    public boolean canHandle(final WorkflowEntity object)
    {
        return object instanceof WorkflowAction && WorkflowActionType.END == ((WorkflowAction)object).getActionType();
    }
    ...
    public Node render(final WorkflowEntity workflowEntity)
    {
        ...
        final String group = getActionStatus(model).orElse(WorkflowDesignerGroup.END_ACTION.getValue());
        return new Node.Builder().withId(String.valueOf(getKeyGenerator().generateFor(model))).withShape("image")
                        ...
                        .withGroup(group).withImage(getWorkflowEntityImageCreator().createSvgImage(model, VELOCITY_TEMPLATE_LOCATION,
                                        VELOCITY_IE_TEMPLATE_LOCATION, getCssClass(model)))
