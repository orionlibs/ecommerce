...
public class DefaultWorkflowDetailsSummaryFlowRenderer extends AbstractSummaryViewItemWithIconRenderer<WorkflowModel>
{
    public static final String LABEL_WORKFLOW_FLOW = "collaboration.workflow.details.summary.flow.value";
    @WireVariable
    ...
    {
        return getIconStatusSClass("workflows", "flow");
    }
    ...
    {
        final int steps = data.getActions().size();
        final int decisions = workflowFacade.countDecisions(data);
        final Label label = new Label(Labels.getLabel(LABEL_WORKFLOW_FLOW, new Integer[]
                        {steps, decisions}));
