...
class SqlServerStrategy extends AbstractStrategy
{
    public SqlServerStrategy(PersistenceInformation persistenceInfo, String columnName, String defaultValue)
    {
    ...
    {
        DDLStatement dDLStatement = new DDLStatement("alter table " + getDeploymentsTableName() + " add " + getColumnName() + " NVARCHAR(255) null");
    ...
    {
        DDLStatement dDLStatement = new DDLStatement("alter table " + getDeploymentsTableName() + " alter column " + getColumnName() + " NVARCHAR(255) not null;");
        return (Iterable<SqlStatement>)ImmutableList.of(dDLStatement);
    }
    ...
    {
        String query = "select distinct CONSTRAINT_NAME from INFORMATION_SCHEMA.KEY_COLUMN_USAGE where OBJECTPROPERTY(OBJECT_ID(constraint_name), 'IsPrimaryKey') = 1 and UPPER(table_name) = UPPER('" + getDeploymentsTableName() + "') and table_schema=SCHEMA_NAME()";
