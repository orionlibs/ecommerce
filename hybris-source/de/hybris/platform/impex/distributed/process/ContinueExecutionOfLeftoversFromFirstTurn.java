...
class ContinueExecutionOfLeftoversFromFirstTurn extends AbstractImportProcessExecutionAnalysisContext
{
    private final DistributedProcessModel process;
    private final ProcessExecutionId nextTurn;


    public ContinueExecutionOfLeftoversFromFirstTurn(DistributedProcessModel process, ProcessExecutionId nextTurn, ModelService modelService, FlexibleSearchService flexibleSearchService)
    {
    ...
    private static final String LEFTOVER_BATCHES_FROM_FIRST_ROUND_QUERY = ROWS_SELECT_CLAUSE
                    + "from {ImportBatch as input left join ImportBatch as result on ({input.id}={result.id} and {input.executionId}={result.executionId} and {result.type}=?resultType and {result.process}=?process)} where {input.type}=?inputType and {input.process}=?process and ({result.pk} is null or {result.remainingWorkLoad} > 0)";
    ...
    public Stream<DistributedProcessHandler.ModelWithDependencies<BatchModel>> nextExecutionInputBatches()
    {
        ...
        return queryForBatchRows(LEFTOVER_BATCHES_FROM_FIRST_ROUND_QUERY, (Map)immutableMap)
