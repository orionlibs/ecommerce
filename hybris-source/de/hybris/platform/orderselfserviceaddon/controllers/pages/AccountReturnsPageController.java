...
public class AccountReturnsPageController extends AbstractSearchPageController
{
    ...
    private static final String RETURNLABEL_DOWNLOAD_HTML = "returnLabelDownloadHtml";
    private static final Pattern urlSplit = Pattern.compile("[?]");
                    ...
                    final Model model, final RedirectAttributes redirectModel) throws CMSItemNotFoundException
    {
        ...
        if(isUserValidForOrder(returnRequestDetails.getOrder()))
        {
            try
            {
                ...
                String htmlDownloadUrl = returnRequestDetails.getReturnLabelDownloadUrl();
                if(htmlDownloadUrl != null && htmlDownloadUrl.indexOf('?') >= 0)
                {
                    htmlDownloadUrl = urlSplit.matcher(htmlDownloadUrl).replaceFirst(".html?");
                }
                model.addAttribute(RETURNLABEL_DOWNLOAD_HTML, htmlDownloadUrl);
