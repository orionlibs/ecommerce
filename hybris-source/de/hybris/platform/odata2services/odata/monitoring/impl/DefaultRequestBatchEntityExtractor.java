...
{
    private static final Logger LOG = Log.getLogger(DefaultRequestBatchEntityExtractor.class);
    ...
    protected List<RequestBatchEntity> extractBatchRequests(final ODataContext context)
    {
        ...
        {
            LOG.error("Failed to parse batch request for monitoring purposes. Persisting all batches as single request", e);
    ...
    protected List<BatchRequestPart> getRequestParts(final ODataContext context, final BatchParser batchParser)
    {
        ...
        {
            LOG.error("Error while parsing batch parts from request");
            throw new InboundMonitoringException(e);
        }
    }
