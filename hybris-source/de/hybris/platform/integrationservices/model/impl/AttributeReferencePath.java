...
class AttributeReferencePath implements ReferencePath
{
    ...
    AttributeReferencePath(final List<TypeAttributeDescriptor> attributes)
    {
        ...
        attributeChain = List.copyOf(attributes);
    }
    ...
    /**
     * {@inheritDoc}
     * @return conventional property path presentation of this property path. If this path is empty, an empty string is returned;
     * otherwise a Java Bean property path, e.g. {@code "catalogVersion.catalog.id"}.<br/>
     * Collection attributes are presented as indexed property, for example {@code "products[0]"}.<br/>
     * Map attributes are presented as value of the first element, i.e. {@code "languages.entrySet[0].value"}<br/>
     * Localized attributes are always represented as value in English locale, i.e. {@code "name['en']"}
     */
    @Override
    public String toPropertyPath()
    {
        final String path = attributeChain.stream()
                        .map(this::asPathElement)
                        .reduce("", (p, n) -> p + "." + n);
        return path.substring(1);
    }


    private String asPathElement(final TypeAttributeDescriptor attribute)
    {
        final String qualifier = attribute.getQualifier();
        return attribute.isCollection() ? (qualifier + "[0]") : qualifier;
    }
