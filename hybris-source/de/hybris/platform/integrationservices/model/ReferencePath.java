...
public interface ReferencePath
{
    ...
    /**
     * Presents this path as conventional Java Bean property path, in which sequence accessor property calls is separated by "."
     * and each next property in the sequence is called on the result of the previous property invocation.<br/>
     * This is purely a presentation of the path and does not guarantee to work when applied to a Java Bean. That is, the path may
     * not check for possible {@code null} values of some properties in the middle of the path, existence of indexed properties, etc.
     *
     * @return conventional property path presentation of this property path. If this path is empty, an empty string is returned;
     * otherwise a Java Bean property path, e.g. {@code "catalogVersion.catalog.id"}.
     */
