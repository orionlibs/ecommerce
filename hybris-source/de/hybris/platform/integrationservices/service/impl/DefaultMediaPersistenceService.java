...
{
    private static final Logger LOG = Log.getLogger(DefaultMediaPersistenceService.class);
    ...
    private <T extends IntegrationApiMediaModel> T persistNonNullPayload(final InputStream payload, final Class<T> mediaType)
    {
        try
        {
            ...
            LOG.debug("Persisting media with name '{}'", mediaName);
            final T media = createMediaModel(mediaName, mediaType);
            getMediaService().setStreamForMedia(media, payload);
            return media;
        }
        catch(final MediaIOException | IllegalArgumentException e)
        {
            LOG.error("Failed persisting media", e);
            return null;
        }
    }
