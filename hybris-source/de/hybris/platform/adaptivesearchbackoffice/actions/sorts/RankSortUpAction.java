...
public class RankSortUpAction extends AbstractComponentWidgetAdapterAware implements CockpitAction<AbstractEditorData, Object>
{
    @Resource
    ...
    public ActionResult<Object> perform(ActionContext<AbstractEditorData> ctx)
    {
        ...
        if(sort instanceof de.hybris.platform.adaptivesearch.model.AsPromotedSortModel)
        {
            this.asConfigurationService.rerankConfiguration((AbstractAsConfigurationModel)searchConfiguration, "promotedSorts", sort.getUid(), -1);
        }
        else if(sort instanceof de.hybris.platform.adaptivesearch.model.AsSortModel)
        {
            this.asConfigurationService.rerankConfiguration((AbstractAsConfigurationModel)searchConfiguration, "sorts", sort.getUid(), -1);
        }
        else if(sort instanceof de.hybris.platform.adaptivesearch.model.AsExcludedSortModel)
        {
            this.asConfigurationService.rerankConfiguration((AbstractAsConfigurationModel)searchConfiguration, "excludedSorts", sort.getUid(), -1);
        }
        else
        {
    ...
    {
        AbstractEditorData data = (AbstractEditorData)ctx.getData();
        if(data == null || !data.isRankUpAllowed())
        {
            return false;
        }
        Object model = data.getModel();
        if(!(model instanceof de.hybris.platform.adaptivesearch.model.AsPromotedSortModel) && !(model instanceof de.hybris.platform.adaptivesearch.model.AsSortModel) && !(model instanceof de.hybris.platform.adaptivesearch.model.AsExcludedSortModel))
        {
            return false;
        }
