...
public class ResetSortAction extends AbstractComponentWidgetAdapterAware implements CockpitAction<AbstractEditorData, Object>
{
    @Resource
    ...
    public ActionResult<Object> perform(ActionContext<AbstractEditorData> ctx)
    {
        ...
        {
            this.asConfigurationService.moveConfiguration((AbstractAsConfigurationModel)searchConfiguration, "promotedSorts", "sorts", sort.getUid());
        }
        else if(sort instanceof de.hybris.platform.adaptivesearch.model.AsExcludedSortModel)
        {
            this.asConfigurationService.moveConfiguration((AbstractAsConfigurationModel)searchConfiguration, "excludedSorts", "sorts", sort.getUid());
        }
        else
        {
    ...
    public boolean canPerform(ActionContext<AbstractEditorData> ctx)
    {
        ...
        if(!(model instanceof de.hybris.platform.adaptivesearch.model.AsPromotedSortModel) && !(model instanceof de.hybris.platform.adaptivesearch.model.AsExcludedSortModel))
        {
            return false;
        }
