...
public class PromoteFacetAction extends AbstractComponentWidgetAdapterAware implements CockpitAction<AbstractEditorData, Object>
{
    @Resource
    ...
    public ActionResult<Object> perform(ActionContext<AbstractEditorData> ctx)
    {
        ...
        if(facet instanceof de.hybris.platform.adaptivesearch.model.AsFacetModel)
        {
            this.asConfigurationService.moveConfiguration((AbstractAsConfigurationModel)searchConfiguration, "facets", "promotedFacets", facet.getUid());
        }
        else if(facet instanceof de.hybris.platform.adaptivesearch.model.AsExcludedFacetModel)
        {
            this.asConfigurationService.moveConfiguration((AbstractAsConfigurationModel)searchConfiguration, "excludedFacets", "promotedFacets", facet.getUid());
        }
        else
        {
    ...
    public boolean canPerform(ActionContext<AbstractEditorData> ctx)
    {
        ...
        if(!(model instanceof de.hybris.platform.adaptivesearch.model.AsFacetModel) && !(model instanceof de.hybris.platform.adaptivesearch.model.AsExcludedFacetModel))
        {
            return false;
        }
