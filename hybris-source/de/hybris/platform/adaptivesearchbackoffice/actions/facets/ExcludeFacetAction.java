...
public class ExcludeFacetAction extends AbstractComponentWidgetAdapterAware implements CockpitAction<AbstractEditorData, Object>
{
    @Resource
    ...
    public ActionResult<Object> perform(ActionContext<AbstractEditorData> ctx)
    {
        ...
        {
            this.asConfigurationService.moveConfiguration((AbstractAsConfigurationModel)searchConfiguration, "promotedFacets", "excludedFacets", facet.getUid());
        }
        else if(facet instanceof de.hybris.platform.adaptivesearch.model.AsFacetModel)
        {
            this.asConfigurationService.moveConfiguration((AbstractAsConfigurationModel)searchConfiguration, "facets", "excludedFacets", facet.getUid());
        }
        else
        {
    ...
    public boolean canPerform(ActionContext<AbstractEditorData> ctx)
    {
        ...
        if(!(model instanceof de.hybris.platform.adaptivesearch.model.AsPromotedFacetModel) && !(model instanceof de.hybris.platform.adaptivesearch.model.AsFacetModel))
        {
            return false;
        }
