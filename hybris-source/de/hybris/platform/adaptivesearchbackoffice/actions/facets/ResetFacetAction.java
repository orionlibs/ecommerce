...
public class ResetFacetAction extends AbstractComponentWidgetAdapterAware implements CockpitAction<AbstractEditorData, Object>
{
    @Resource
    ...
    public ActionResult<Object> perform(ActionContext<AbstractEditorData> ctx)
    {
        ...
        {
            this.asConfigurationService.moveConfiguration((AbstractAsConfigurationModel)searchConfiguration, "promotedFacets", "facets", facet.getUid());
        }
        else if(facet instanceof de.hybris.platform.adaptivesearch.model.AsExcludedFacetModel)
        {
            this.asConfigurationService.moveConfiguration((AbstractAsConfigurationModel)searchConfiguration, "excludedFacets", "facets", facet.getUid());
        }
        else
        {
    ...
    public boolean canPerform(ActionContext<AbstractEditorData> ctx)
    {
        ...
        if(!(model instanceof de.hybris.platform.adaptivesearch.model.AsPromotedFacetModel) && !(model instanceof de.hybris.platform.adaptivesearch.model.AsExcludedFacetModel))
        {
            return false;
        }
