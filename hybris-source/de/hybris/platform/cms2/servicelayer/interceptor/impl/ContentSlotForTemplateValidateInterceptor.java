...
public class ContentSlotForTemplateValidateInterceptor implements ValidateInterceptor
{
    ...
    {
        ContentSlotForTemplateModel csmodel = (ContentSlotForTemplateModel)model;
        String position = csmodel.getPosition();
        PageTemplateModel page = csmodel.getPageTemplate();
        ContentSlotModel contentSlot = csmodel.getContentSlot();
        if(page.getPk() == null || contentSlot.getPk() == null)
        {
            return;
        }
        StringBuilder query = new StringBuilder("SELECT COUNT({csfp.PK}) FROM {  ContentSlotForTemplate as csfp JOIN  PageTemplate as ap ON {csfp.pageTemplate}={ap.PK} JOIN  ContentSlot as cs ON {csfp.contentSlot}={cs.PK}} WHERE {csfp.position}=?position AND {ap.PK}=?page AND {cs.PK}=?slot");
        ...
        {
            throw new InterceptorException("position, pageTemplate and content slot must be unique");
        }
    }
