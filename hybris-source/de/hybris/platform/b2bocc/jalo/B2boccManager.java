 ...
 */
package de.hybris.platform.b2bocc.jalo;
 ...
 */
public class B2boccManager extends GeneratedB2boccManager
{
    /** Edit the local|project.properties to change logging behavior (properties 'log4j.*'). */
    private static final Logger LOG = Logger.getLogger(B2boccManager.class.getName());
     ...
     */
    public static B2boccManager getInstance()
    {
        return (B2boccManager)Registry.getCurrentTenant().getJaloConnection().getExtensionManager().getExtension(
                        B2boccConstants.EXTENSIONNAME);
    }
     ...
     */
    public B2boccManager()
    {
        if(LOG.isDebugEnabled())
        {
            LOG.debug("constructor of B2boccManager called.");
        }
    }
    ...
    public void init()
    {
        if(LOG.isDebugEnabled())
        {
            LOG.debug("init() of B2boccManager called. " + getTenant().getTenantID());
        }
    }
    ...
    public void destroy()
    {
        if(LOG.isDebugEnabled())
        {
            LOG.debug("destroy() of B2boccManager called, current tenant: " + getTenant().getTenantID());
        }
    }
