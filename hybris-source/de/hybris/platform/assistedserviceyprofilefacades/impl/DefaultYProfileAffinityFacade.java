...
public class DefaultYProfileAffinityFacade implements YProfileAffinityFacade
{
    ...
    {
        final Optional<Map<String, Affinity>> productAffinitiesMap;
        try
        {
            final Optional<Profile> optionalProfile = getProfileData(YaasyprofileconnectConstants.SCHEMA_COMMERCE_PRODUCT_AFFINITY);
            productAffinitiesMap = optionalProfile.flatMap(profile -> Optional.ofNullable(profile.getInsights()))
                            .flatMap(insights -> Optional.ofNullable(insights.getAffinities()))
                            .flatMap(affinities -> Optional.ofNullable(affinities.getProducts()));
        }
        ...
        if(productAffinitiesMap.isEmpty())
        {
            LOG.warn("Couldn't retrieve product affinity data");
            return new ArrayList<>();
        }
        final List sortedProducts = getSortedAffinities(productAffinitiesMap.get(), productAffinityParameterData.getSizeLimit());
    ...
    {
        final Optional<Map<String, Affinity>> categoryAffinitiesMap;
        try
        {
            final Optional<Profile> optionalProfile = getProfileData(YaasyprofileconnectConstants.SCHEMA_COMMERCE_CATEGORY_AFFINITY);
            categoryAffinitiesMap = optionalProfile.flatMap(profile -> Optional.ofNullable(profile.getInsights()))
                            .flatMap(insights -> Optional.ofNullable(insights.getAffinities()))
                            .flatMap(affinities -> Optional.ofNullable(affinities.getCategories()));
        }
        catch(final NoSuchElementException e)
        {
        ...
        if(categoryAffinitiesMap.isEmpty())
        {
            LOG.warn("Couldn't retrieve category affinity data");
            return new ArrayList<>();
        }
        final List sortedCategories = getSortedAffinities(categoryAffinitiesMap.get(),
                        categoryAffinityParameterData.getSizeLimit());
    ...
    public List<TechnologyUsedData> getDeviceAffinities(final DeviceAffinityParameterData deviceAffinityParameterData)
    {
        ...
        final Optional<Profile> optionalProfile = getProfileData(YaasyprofileconnectConstants.SCHEMA_COMMERCE_DEVICE_AFFINITY);
        if(optionalProfile.isPresent())
        {
            deviceAffinitiesMap = optionalProfile.get().getUserAgents();
        }
        else
        {
        ...
        return Converters.convertAll(
                        deviceAffinitiesMap.entrySet().parallelStream().limit(deviceAffinityParameterData.getSizeLimit())
     ...
     */
    protected Optional<Profile> getProfileData(final String profileFieldsSchema)
    {
        ...
        if(StringUtils.isNotEmpty(profileId))
        {
            ...
            {
                LOG.warn(
                                "Failed to get yaas profile for user {}. Check if yaas configuration is properly defined for AsmProfileDataServiceClient. ",
                                user.getUid(), e);
            }
        }
        return Optional.empty();
     ...
     * @return String
     */
    protected String getProfileId(final UserModel user)
    {
        ...
        {
            LOG.warn(
                            "Failed to get yaas profile reference for user {}. Check if yaas configuration is properly defined for AsmProfileIdentityServiceClient. ",
                            user.getUid(), e);
        }
     ...
     * @return String
     */
    protected List getSortedAffinities(final Map<String, Affinity> profileData, final int sizeLimit)
    {
        final List profileDataList = new ArrayList(profileData.entrySet());
        return (List)profileDataList.parallelStream().sorted(new RecentlyViewedComparator()).limit(sizeLimit)
                        .collect(Collectors.toList());
    }
    ...
    @Required
    public void setAsmProfileDataServiceClientAdapter(final AsmProfileDataServiceClientAdapter asmProfileDataServiceClientAdapter)
    {
    ...
    @Required
    public void setAsmProfileIdentityServiceClientAdapter(
                    final AsmProfileIdentityServiceClientAdapter asmProfileIdentityServiceClientAdapter)
    {
    ...
    @Required
    public void setProductAffinityConverter(
                    final Converter<Map.Entry<String, Affinity>, ProductAffinityData> productAffinityConverter)
    {
    ...
    @Required
    public void setCategoryAffinityConverter(
                    final Converter<Map.Entry<String, Affinity>, CategoryAffinityData> categoryAffinityConverter)
    {
    ...
    @Required
    public void setDeviceAffinityConverter(
                    final Converter<Map.Entry<String, UserAgent>, TechnologyUsedData> deviceAffinityConverter)
    {
