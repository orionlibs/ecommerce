package de.hybris.platform.commerceservices.enums;

import de.hybris.bootstrap.typesystem.HybrisDynamicEnumValueSerializedForm;
import de.hybris.platform.core.HybrisEnumValue;
import java.util.concurrent.ConcurrentHashMap;
import java.util.concurrent.ConcurrentMap;

public class PickupInStoreMode implements HybrisEnumValue
{
    public static final String _TYPECODE = "PickupInStoreMode";
    public static final String SIMPLE_CLASSNAME = "PickupInStoreMode";
    private static final ConcurrentMap<String, PickupInStoreMode> cache = new ConcurrentHashMap<>();
    public static final PickupInStoreMode DISABLED = valueOf("DISABLED");
    public static final PickupInStoreMode BUY_AND_COLLECT = valueOf("BUY_AND_COLLECT");
    public static final PickupInStoreMode RESERVE_AND_COLLECT = valueOf("RESERVE_AND_COLLECT");
    private final String code;
    private final String codeLowerCase;
    private static final long serialVersionUID = 0L;


    private PickupInStoreMode(String code)
    {
        this.code = code.intern();
        this.codeLowerCase = this.code.toLowerCase().intern();
    }


    public boolean equals(Object obj)
    {
        try
        {
            HybrisEnumValue enum2 = (HybrisEnumValue)obj;
            return (this == enum2 || (enum2 != null &&
                            !getClass().isEnum() && !enum2.getClass().isEnum() &&
                            getType().equalsIgnoreCase(enum2.getType()) && getCode().equalsIgnoreCase(enum2.getCode())));
        }
        catch(ClassCastException e)
        {
            return false;
        }
    }


    public String getCode()
    {
        return this.code;
    }


    public String getType()
    {
        return "PickupInStoreMode";
    }


    public int hashCode()
    {
        return this.codeLowerCase.hashCode();
    }


    public String toString()
    {
        return this.code.toString();
    }


    public static PickupInStoreMode valueOf(String code)
    {
        String key = code.toLowerCase();
        PickupInStoreMode result = cache.get(key);
        if(result == null)
        {
            PickupInStoreMode newValue = new PickupInStoreMode(code);
            PickupInStoreMode previous = cache.putIfAbsent(key, newValue);
            result = (previous != null) ? previous : newValue;
        }
        return result;
    }


    private Object writeReplace()
    {
        return new HybrisDynamicEnumValueSerializedForm(getClass(), getCode());
    }
}
