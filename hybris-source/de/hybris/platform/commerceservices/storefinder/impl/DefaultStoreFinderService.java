 ...
 */
public class DefaultStoreFinderService<ITEM extends PointOfServiceDistanceData>
                implements StoreFinderService<ITEM, StoreFinderSearchPageData<ITEM>>
{
    ...
    public PointOfServiceModel getPointOfServiceForName(final BaseStoreModel baseStore, final String name)
    {
        ...
        throw new ModelNotFoundException(
                        "No PointOfService with name " + name + ", type STORE found in baseStore " + baseStore.getUid());
    }
    ...
    @Override
    public List<PointOfServiceModel> getAllPosForCountry(final String countryIsoCode, final BaseStoreModel baseStore)
    {
        return getPointOfServiceDao().getPosForCountry(countryIsoCode, baseStore);
    }


    @Override
    public List<PointOfServiceModel> getAllPosForRegion(final String countryIsoCode, final String regionIsoCode,
                    final BaseStoreModel baseStore)
    {
        return getPointOfServiceDao().getPosForRegion(countryIsoCode, regionIsoCode, baseStore);
    }
    // ----------
        ...
        if(CollectionUtils.isNotEmpty(results) && centerPoint != null)
        {
            ...
            for(final ITEM result : results)
            {
                ...
                if(latitude != null && longitude != null)
                {
                    ...
                    {
                        LOG.debug(
                                        " bounds coords NE(" + northEast + "), SW(" + southWest + ") after a point (" + latitude + "," + longitude
                                                        + ")");
                    }
                }
            }
