...
{
    private static final Logger LOG = Logger.getLogger(DefaultCustomerAccountDao.class);
    // Order Queries
                    ...
                    final OrderStatus[] status)
    {
        ...
        if(ArrayUtils.isNotEmpty(status))
        {
            ...
            query = createQuery(FIND_ORDERS_BY_CUSTOMER_STORE_QUERY_AND_STATUS + getFindOrdersAdditionalFilter(), filterClause);
        }
        else
        {
        ...
        if(ArrayUtils.isNotEmpty(status))
        {
            ...
            sortQueries = Arrays.asList(
                            createSortQueryData("byDate",
                                            createQuery(FIND_ORDERS_BY_CUSTOMER_STORE_QUERY_AND_STATUS + getFindOrdersAdditionalFilter(),
                                                            filterClause, SORT_ORDERS_BY_DATE)),
                            createSortQueryData("byOrderNumber",
                                            createQuery(FIND_ORDERS_BY_CUSTOMER_STORE_QUERY_AND_STATUS + getFindOrdersAdditionalFilter(),
                                                            filterClause, SORT_ORDERS_BY_CODE)));
        }
        else
        {
            sortQueries = Arrays
                            .asList(
                                            createSortQueryData("byDate",
                                                            createQuery(FIND_ORDERS_BY_CUSTOMER_STORE_QUERY + getFindOrdersAdditionalFilter(),
                                                                            filterClause, SORT_ORDERS_BY_DATE)),
                                            createSortQueryData("byOrderNumber",
                                                            createQuery(FIND_ORDERS_BY_CUSTOMER_STORE_QUERY + getFindOrdersAdditionalFilter(),
                                                                            filterClause, SORT_ORDERS_BY_CODE)));
    ...
    protected String getFindOrdersAdditionalFilter()
    {
        try
        {
            if(getConfigurationService().getConfiguration().getBoolean(
                            CommerceServicesConstants.FIND_ORDERS_ADDITIONAL_FILTER_ENABLED, false))
            {
