...
public class DefaultCommerceCartMergingStrategy extends AbstractCommerceCartStrategy implements CommerceCartMergingStrategy
{
    ...
    private List<CommerceCartMergingMethodHook> commerceCartMergingMethodHooks;
                    ...
                    throws CommerceCartMergingException
    {
        ...
        beforeCartMerge(fromCart, toCart);
        validationBeforeMergeResult(fromCart);
        // copy entry group
        ...
        // after merge
        afterCartMerge(fromCart, toCart);
    ...
    protected void validationBeforeMergeResult(final CartModel fromCart)
    {
        validateParameterNotNullStandardMessage("fromCartEntries", fromCart.getEntries());
        final List<AbstractOrderEntryModel> entriesToRemove = fromCart.getEntries().stream()
                        .filter(e -> e.getGiveAway().booleanValue()).collect(Collectors.toList());
        getModelService().removeAll(entriesToRemove);
        final List<AbstractOrderEntryModel> fromEntries = fromCart.getEntries().stream()
                        .filter(e -> !e.getGiveAway().booleanValue()).collect(Collectors.toList());
        fromCart.setEntries(fromEntries);
    }
    ...
    protected void beforeCartMerge(final CartModel fromCart, final CartModel toCart)
    {
        if(getCommerceCartMergingMethodHooks() != null && getConfigurationService().getConfiguration().getBoolean(
                        CommerceServicesConstants.CARTMERGEHOOK_ENABLED, true))
        {
            for(final CommerceCartMergingMethodHook commerceCartMergingMethodHook : getCommerceCartMergingMethodHooks())
            {
                commerceCartMergingMethodHook.beforeCartMerge(fromCart, toCart);
            }
        }
    }


    protected void afterCartMerge(final CartModel fromCart, final CartModel toCart)
    {
        if(getCommerceCartMergingMethodHooks() != null && getConfigurationService().getConfiguration().getBoolean(
                        CommerceServicesConstants.CARTMERGEHOOK_ENABLED, true))
        {
            for(final CommerceCartMergingMethodHook commerceCartMergingMethodHook : getCommerceCartMergingMethodHooks())
            {
                commerceCartMergingMethodHook.afterCartMerge(fromCart, toCart);
            }
        }
    }
    ...
    public List<CommerceCartMergingMethodHook> getCommerceCartMergingMethodHooks()
    {
        return commerceCartMergingMethodHooks;
    }


    @Required
    public void setCommerceCartMergingMethodHooks(final List<CommerceCartMergingMethodHook> commerceCartMergingMethodHooks)
    {
        this.commerceCartMergingMethodHooks = commerceCartMergingMethodHooks;
    }
