...
public class DefaultSiteMessageService implements SiteMessageService
{
    ...
    /**
     * @deprecated since 1905, replace with the next createMessage.
     */
    ...
    @Override
    public SiteMessageModel createMessage(final String subject, final String body, final SiteMessageType type,
                    final ItemModel externalItem, final NotificationType notificationType)
    {
        final SiteMessageModel message = getModelService().create(SiteMessageModel.class);
        message.setUid(getSiteMessageUidGenerator().generate().toString());
