...
public class DefaultProxyService implements ProxyService
{
    ...
    {
        String url = this.orbeonAddress + ORBEON_PREFIX + "/fr";
        if(withEmptyData)
        {
            url = url + SERVICE_PREFIX;
        }
        url = url + "/" + applicationId + "/" + formId;
    ...
    public String rewriteURL(final String applicationId, final String formId, final String formDataId, final boolean editable)
                    throws MalformedURLException
    {
        return rewriteURL(applicationId, formId, formDataId, editable, false);
    }
