...
public class UpdateAgentGroupEditorDecorator extends AbstractCockpitEditorRenderer<String>
{
    @Resource
    ...
    public void render(Component parent, EditorContext<String> editorContext, EditorListener<String> editorListener)
    {
        ...
        ancestorEditor.addEventListener("onValueChanged", event -> {
            CsAgentGroupModel group = (CsAgentGroupModel)event.getData();
            this.modelService.refresh(ticket);
            this.ticketBusinessService.assignTicketToGroup(ticket, group);
            this.modelService.refresh(ticket);
