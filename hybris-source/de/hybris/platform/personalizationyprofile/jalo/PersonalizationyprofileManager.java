...
/**
 * This is the extension manager of the Personalizationyprofile extension.
 */
public class PersonalizationyprofileManager extends GeneratedPersonalizationyprofileManager
{
    /** Edit the local|project.properties to change logging behavior (properties 'log4j.*'). */
    private static final Logger LOG = Logger.getLogger(PersonalizationyprofileManager.class.getName());
     ...
     */
    public static PersonalizationyprofileManager getInstance()
    {
        return (PersonalizationyprofileManager)Registry.getCurrentTenant().getJaloConnection().getExtensionManager()
                        .getExtension(PersonalizationyprofileConstants.EXTENSIONNAME);
    }
     ...
     */
    public PersonalizationyprofileManager()
    {
        if(LOG.isDebugEnabled())
        {
            LOG.debug("constructor of PersonalizationyprofileManager called.");
        }
    }
    ...
    public void init()
    {
        if(LOG.isDebugEnabled())
        {
            LOG.debug("init() of PersonalizationyprofileManager called. " + getTenant().getTenantID());
        }
    }
    ...
    public void destroy()
    {
        if(LOG.isDebugEnabled())
        {
            LOG.debug("destroy() of PersonalizationyprofileManager called, current tenant: " + getTenant().getTenantID());
        }
    }
