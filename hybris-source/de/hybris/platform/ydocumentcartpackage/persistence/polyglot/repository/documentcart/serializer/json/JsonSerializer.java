...
public class JsonSerializer implements Serializer
{
    ...
    protected static final String SET = Set.class.getSimpleName().toUpperCase();
    protected static final String MAP = Map.class.getSimpleName().toUpperCase();
    protected static final String MAP_KEY = "_k";
    protected static final String MAP_VALUE = "_v";
    ...
    protected static class DocumentSerializer extends StdSerializer<DocumentWrapper>
    {
                ...
                entity.forEveryAttribute((key, value) -> {
                    ...
                    catch(final IOException e)
                    {
                        throw new WrappedIOException(
                                        new IOException("error while serializing field " + fieldName + ":" + e.getMessage(), e));
                    }
                    catch(final Exception e)
                    {
                        throw new IllegalStateException("error while serializing field " + fieldName + ": " + e.getMessage(), e);
                    }
                });
                gen.writeEndObject();
            }
        ...
        private void writeMap(final Map<?, ?> value, final JsonGenerator gen) throws IOException
        {
            ...
            {
                gen.writeStartObject();
                gen.writeFieldName(MAP_KEY);
                writeEntityField(e.getKey(), gen);
                gen.writeFieldName(MAP_VALUE);
                writeEntityField(e.getValue(), gen);
                gen.writeEndObject();
            }
            gen.writeEndArray();
        }
    ...
    protected static class DocumentDeserializer extends StdDeserializer<Document>
    {
        ...
        private Object readMapField(final ArrayNode array)
        {
            if(array.isEmpty())
            {
            ...
            return StreamSupport.stream(Spliterators.spliterator(array.elements(), array.size(), 0), false)
                            .map(this::readMapEntry).collect(Collectors.toMap(Pair::getLeft, Pair::getRight));
        }


        private Pair<?, ?> readMapEntry(final JsonNode entryNode)
        {
            return Pair.of(readEntityField(entryNode.get(MAP_KEY)), readEntityField(entryNode.get(MAP_VALUE)));
        }
    }
