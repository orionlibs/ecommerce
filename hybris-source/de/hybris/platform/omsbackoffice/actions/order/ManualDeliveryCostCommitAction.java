...
public class ManualDeliveryCostCommitAction extends AbstractComponentWidgetAdapterAware implements CockpitAction<OrderModel, OrderModel>
{
    ...
    public ActionResult<OrderModel> perform(ActionContext<OrderModel> actionContext)
    {
        ...
        if(actionContext != null && actionContext.getData() != null)
        {
            ...
            getNotificationService().notifyUser(this.notificationService.getWidgetNotificationSource(actionContext), "JustMessage", NotificationEvent.Level.SUCCESS, new Object[] {actionContext
                            .getLabel("action.manualdeliverycostcommit.success")});
        }
        return actionResult;
    }
    ...
    protected void executeManualDeliveryCostTaxCommitOperation(OrderModel order)
    {
        order.getOrderProcess().stream()
                        .filter(process -> process.getCode().startsWith(order.getStore().getSubmitOrderProcessCode())).forEach(filteredProcess -> getBusinessProcessService().triggerEvent(filteredProcess.getCode() + "_ManualCommitDeliveryCostEvent"));
        LOG.info("Delivery Cost Tax Commit Manual Release completed. {} triggered.", "ManualCommitDeliveryCostEvent");
