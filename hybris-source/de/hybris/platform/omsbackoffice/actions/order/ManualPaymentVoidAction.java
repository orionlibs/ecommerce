...
public class ManualPaymentVoidAction extends AbstractComponentWidgetAdapterAware implements CockpitAction<OrderModel, OrderModel>
{
    ...
    private static final Logger LOG = Logger.getLogger(ManualPaymentVoidAction.class);
    @Resource
    ...
    public ActionResult<OrderModel> perform(ActionContext<OrderModel> actionContext)
    {
        ...
        if(actionContext != null && actionContext.getData() != null)
        {
            ...
            executeManualPaymentVoidOperation(order);
            ...
            this.notificationService.notifyUser(this.notificationService.getWidgetNotificationSource(actionContext), "JustMessage", NotificationEvent.Level.SUCCESS, new Object[] {actionContext
                            .getLabel("action.manualpaymentvoid.success")});
    ...
    public boolean canPerform(ActionContext<OrderModel> ctx)
    {
        ...
        return (order != null && OrderStatus.PAYMENT_NOT_VOIDED.equals(order.getStatus()));
    }
    ...
    protected void executeManualPaymentVoidOperation(OrderModel order)
    {
        order.getOrderProcess().stream()
                        .filter(process -> process.getCode().startsWith(order.getStore().getSubmitOrderProcessCode())).forEach(filteredProcess -> getBusinessProcessService().triggerEvent(filteredProcess.getCode() + "_ManualVoidPaymentEvent"));
        LOG.info(String.format("Payment Void Manual Release completed. %s triggered.", new Object[] {"ManualVoidPaymentEvent"}));
