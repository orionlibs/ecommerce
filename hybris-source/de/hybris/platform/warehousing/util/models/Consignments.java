...
public class Consignments extends AbstractItems<ConsignmentModel>
{
    ...
    public ConsignmentModel Camera_ShippedFromMontrealToMontrealNancyHome(ConsignmentStatus status, Long quantity)
    {
        return (ConsignmentModel)getOrSaveAndReturn(() -> (ConsignmentModel)getWarehousingConsignmentDao().getByCode("camera-shipped-from-montreal-to-montreal-nancy-home"), () -> {
            ConsignmentEntryModel entry = Camera(quantity);
            ConsignmentModel consignment = ConsignmentModelBuilder.aModel().withCode("camera-shipped-from-montreal-to-montreal-nancy-home").withWorkflowCode("dummyCode").withDeliveryMode(getDeliveryModes().Regular()).withWarehouse(getWarehouses().Montreal())
                            .withShippingAddress(getAddresses().MontrealNancyHome()).withStatus(status).withEntries(new ConsignmentEntryModel[] {entry}).withOrder((OrderModel)entry.getOrderEntry().getOrder()).build();
    ...
    public ConsignmentModel Camera_ShippedFromBostonToMontrealNancyHome(ConsignmentStatus status, Long quantity)
    {
        return (ConsignmentModel)getOrSaveAndReturn(() -> (ConsignmentModel)getWarehousingConsignmentDao().getByCode("camera-shipped-from-boston-to-montreal-nancy-home"), () -> {
            ConsignmentEntryModel entry = Camera(quantity);
            ConsignmentModel consignment = ConsignmentModelBuilder.aModel().withCode("camera-shipped-from-boston-to-montreal-nancy-home").withWorkflowCode("dummyCode").withDeliveryMode(getDeliveryModes().Regular()).withWarehouse(getWarehouses().Boston())
                            .withShippingAddress(getAddresses().MontrealNancyHome()).withStatus(status).withEntries(new ConsignmentEntryModel[] {entry}).withOrder((OrderModel)entry.getOrderEntry().getOrder()).build();
            entry.setConsignment(consignment);
            return consignment;
        });
    ...
    protected ConsignmentEntryModel Camera(Long quantity)
    {
