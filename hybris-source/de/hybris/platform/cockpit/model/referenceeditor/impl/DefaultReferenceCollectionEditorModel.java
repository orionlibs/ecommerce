...
import de.hybris.platform.cockpit.model.referenceeditor.AbstractCollectionEditorModel;
...
@Deprecated
public class DefaultReferenceCollectionEditorModel extends AbstractCollectionEditorModel
{
    private static final Logger log = LoggerFactory.getLogger(DefaultReferenceCollectionEditorModel.class);
    ...
    public void setCollectionItems(List<Object> collectionItems)
    {
        if(this.collectionItems != collectionItems && !this.collectionItems.equals(collectionItems))
        {
            ...
            if(collectionItems != null)
            {
                ...
                this.referenceSelectorModel.addTemporaryItems(new ArrayList(getTypeService().wrapItems(collectionItems)));
            }
    ...
    public boolean removeCollectionItems(Collection indexes)
    {
        ...
        {
            if(this.collectionItems.size() <= ((Integer)index).intValue())
            {
                ret = false;
            }
            if(!this.collectionItems.remove(index))
            {
                ret = false;
            }
        }
    ...
    {
        this.referenceSelectorModel = new DefaultReferenceSelectorModel(rootType);
        this.referenceSelectorModel.setMultiple(Boolean.TRUE.booleanValue());
