 ...
 */
package de.hybris.platform.cmsocc.jalo;

import de.hybris.platform.cmsocc.constants.CmsoccConstants;
import de.hybris.platform.cmsocc.jalo.GeneratedCmsoccManager;
 ...
 */
public class CmsoccManager extends GeneratedCmsoccManager
{
    /** Edit the local|project.properties to change logging behavior (properties 'log4j.*'). */
    private static final Logger LOG = Logger.getLogger(CmsoccManager.class.getName());
     ...
     */
    public CmsoccManager() // NOPMD
    {
        if(LOG.isDebugEnabled())
        {
            LOG.debug("constructor of CmsoccManager called.");
        }
    }
     ...
     */
    public static CmsoccManager getInstance()
    {
        return (CmsoccManager)Registry.getCurrentTenant().getJaloConnection().getExtensionManager()
                        .getExtension(CmsoccConstants.EXTENSIONNAME);
    }
    ...
    public void init()
    {
        if(LOG.isDebugEnabled())
        {
            LOG.debug("init() of CmsoccManager called. " + getTenant().getTenantID());
        }
    }
    ...
    public void destroy()
    {
        if(LOG.isDebugEnabled())
        {
            LOG.debug("destroy() of CmsoccManager called, current tenant: " + getTenant().getTenantID());
        }
    }
