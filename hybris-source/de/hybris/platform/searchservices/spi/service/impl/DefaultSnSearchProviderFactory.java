...
public class DefaultSnSearchProviderFactory implements SnSearchProviderFactory, ApplicationContextAware, InitializingBean
{
    ...
    @Override
    public <T extends AbstractSnSearchProviderConfiguration> SnSearchProvider<T> getSearchProviderForConfiguration(
                    final AbstractSnSearchProviderConfiguration configuration)
    {
        Objects.requireNonNull(configuration, "context must not be null");
        return (SnSearchProvider<T>)getSearchProviderMappingForConfiguration(configuration).getSearchProvider();
    }
