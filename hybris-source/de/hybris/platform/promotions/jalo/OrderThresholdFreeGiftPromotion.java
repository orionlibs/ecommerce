...
public class OrderThresholdFreeGiftPromotion extends GeneratedOrderThresholdFreeGiftPromotion
{
    private static final Logger LOG = Logger.getLogger(OrderThresholdFreeGiftPromotion.class.getName());
        ...
        if(checkRestrictions(ctx, promoContext))
        {
            ...
            if(threshold != null)
            {
                ...
                if(orderSubtotalAfterDiscounts >= threshold.doubleValue())
                {
                    ...
                    {
                        LOG.debug("(" + getPK() + ") evaluate: Subtotal " + orderSubtotalAfterDiscounts + ">" + threshold + ".  Creating a free gift action.");
                    }
                    PromotionResult result = PromotionsManager.getInstance().createPromotionResult(ctx, (AbstractPromotion)this, promoContext
                                    .getOrder(), 1.0F);
                    Product product = getGiftProduct(ctx);
                    result.addAction(ctx, (AbstractPromotionAction)PromotionsManager.getInstance().createPromotionOrderAddFreeGiftAction(ctx, product, result));
                ...
                else
                {
                    if(LOG.isDebugEnabled())
                    {
                        LOG.debug("(" + getPK() + ") evaluate: Subtotal " + orderSubtotalAfterDiscounts + "<" + threshold + ".  Skipping free gift action.");
                    }
    ...
    protected void buildDataUniqueKey(SessionContext ctx, StringBuilder builder)
    {
        ...
        builder.append(getGiftProduct(ctx).getCode(ctx)).append('|');
