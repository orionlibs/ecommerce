...
public class InboundIntegrationObjectInterceptor implements HandlerInterceptor
{
    ...
    private IntegrationObjectService integrationObjectService = null;
    ...
    private boolean isForExistingIntegrationObject(final HttpServletRequest request)
    {
        ...
        try
        {
            ...
            final boolean isInbound = IntegrationType.INBOUND.equals(ioModel.getIntegrationType());
            if(!isInbound)
            {
                LOG.debug("IntegrationObject {} is not INBOUND and cannot be accessed", ioCode);
            }
            return isInbound;
        }
        catch(final ModelNotFoundException | IllegalArgumentException e)
        {
            LOG.debug("IntegrationObject not found for code: '{}'", ioCode, e);
            return false;
        }
    }
