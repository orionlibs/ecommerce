 ...
 */
package de.hybris.platform.acceleratorocc.jalo;

import de.hybris.platform.acceleratorocc.constants.AcceleratoroccConstants;
 ...
 */
public class AcceleratoroccManager extends GeneratedAcceleratoroccManager
{
    /**
     * Edit the local|project.properties to change logging behavior (properties 'log4j.*').
     */
    private static final Logger LOG = Logger.getLogger(AcceleratoroccManager.class.getName());
     ...
     */
    public AcceleratoroccManager()
    {
        if(LOG.isDebugEnabled())
        {
            LOG.debug("constructor of AcceleratoroccManager called.");
        }
    }
     ...
     */
    public static AcceleratoroccManager getInstance()
    {
        return (AcceleratoroccManager)Registry.getCurrentTenant().getJaloConnection().getExtensionManager()
                        .getExtension(AcceleratoroccConstants.EXTENSIONNAME);
    }
    ...
    public void init()
    {
        if(LOG.isDebugEnabled())
        {
            LOG.debug("init() of AcceleratoroccManager called. " + getTenant().getTenantID());
        }
    }
    ...
    public void destroy()
    {
        if(LOG.isDebugEnabled())
        {
            LOG.debug("destroy() of AcceleratoroccManager called, current tenant: " + getTenant().getTenantID());
        }
    }
