...
{
    private static final Logger LOG = LoggerFactory.getLogger(DefaultB2BUnitService.class);
    private static final int BRANCH_INFO_ROOT_UNIT_IDX = 0;
    private static final int BRANCH_INFO_BRANCH_IDX = 1;
    private static final int BRANCH_INFO_UNIT_IDX = 2;
    ...
    protected void toggleUnit(final B2BUnitModel b2bUnitModel, final Boolean enable)
    {
        ...
        final Set<B2BCustomerModel> customers = new HashSet<>(getB2BCustomers(b2bUnitModel));
    ...
    {
        return new HashSet<>(getPrincipalGroupMembersDao().findHierarchyMembersByType((Set)units, B2BUnitModel.class));
    }
    ...
    {
        return new HashSet<>(getPrincipalGroupMembersDao().findAllMembersByType(unit, B2BUnitModel.class));
    }
    ...
    {
        return new HashSet<>(getPrincipalGroupMembersDao().findAllMembersByType(unit, B2BCustomerModel.class));
    }
    ...
    protected Set<B2BUnitModel> getBranchHierarchy(final B2BUnitModel parent)
    {
        ...
        do
        {
            ...
            final Set<B2BUnitModel> currentLevel = new HashSet<>(getB2BHierarchyUnits(toQuery));
            // cycle safe
    ...
    {
        final Set<B2BUnitModel> decendants = new HashSet<>();
        final Set<B2BUnitModel> rootDecendants = new HashSet<>(getB2BUnits(parent));
    ...
    public void updateBranchInSession(final Session session, final UserModel currentUser)
    {
        if(currentUser instanceof B2BCustomerModel)
        {
            ...
            getSessionService().setAttribute(B2BConstants.CTX_ATTRIBUTE_ROOTUNIT, branchInfo[BRANCH_INFO_ROOT_UNIT_IDX]);
            getSessionService().setAttribute(B2BConstants.CTX_ATTRIBUTE_BRANCH, branchInfo[BRANCH_INFO_BRANCH_IDX]);
            getSessionService().setAttribute(B2BConstants.CTX_ATTRIBUTE_UNIT, branchInfo[BRANCH_INFO_UNIT_IDX]);
            getSessionService().setAttribute(Europe1Constants.PARAMS.UPG, branchInfo[3]);
            if(LOG.isDebugEnabled())
            {
                LOG.debug("Updating branch for user: {} - rootunit: {} - branch: {} - unit: {}", currentUser,
                                branchInfo[BRANCH_INFO_ROOT_UNIT_IDX], branchInfo[BRANCH_INFO_BRANCH_IDX], branchInfo[BRANCH_INFO_UNIT_IDX]);
            }
        }
        else
        {
            if(LOG.isDebugEnabled())
            {
                LOG.debug("currentUser: {} is not an instance of B2BCustomerModel. Will not update branch in session.",
                                currentUser);
            }
        }
    }
