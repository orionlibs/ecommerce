...
public final class DefaultIntegrationObjectImpexGenerator implements IntegrationObjectImpexGenerator
{
    ...
    private int longestIntegrationObjectItemCodeLength;
    private int longestIntegrationObjectItemTypeCodeLength;
    ...
    public String generateImpex(final IntegrationObjectModel selectedIntegrationObject)
    {
        ...
        calculateLongestStrings(integrationObjectItems);
    ...
    private void constructIntegrationObjectItemLine(final IntegrationObjectItemModel item)
    {
        ...
        impexBuilder.append(integrationObjectCode);
        impexBuilder.append(HEADER_TAB_DELIMITER);
        impexBuilder.append(itemCode);
        addWhitespace(longestIntegrationObjectItemCodeLength, itemCode.length(), impexBuilder);
        impexBuilder.append(HEADER_TAB_DELIMITER);
        impexBuilder.append(itemTypeCode);
        addWhitespace(longestIntegrationObjectItemTypeCodeLength, itemTypeCode.length(), impexBuilder);
        impexBuilder.append(HEADER_TAB_DELIMITER);
    ...
    private void constructIntegrationObjectItemAttributeLine(final int[] lengths, final IntegrationObjectItemModel item)
    {
        ...
        final String currentItemTypeCode = item.getType().getCode();
        for(final IntegrationObjectItemAttributeModel attribute : item.getAttributes())
        {
            ...
            final String attributeDescriptor = currentItemTypeCode + ":" + attribute.getAttributeDescriptor().getQualifier();
                    ...
                    final IntegrationObjectItemModel item)
    {
        ...
        for(final IntegrationObjectItemClassificationAttributeModel classificationAttribute : item.getClassificationAttributes())
        {
            ...
            addWhitespace(longestIntegrationObjectItemCodeLength, referenceType.length(), impexBuilder);
            impexBuilder.append(HEADER_TAB_DELIMITER);
            impexBuilder.append(autocreate);
            impexBuilder.append(ENDL);
        }
    ...
    private void constructIntegrationObjectItemSection(final String integrationObjectItemCode)
    {
        ...
        addWhitespace(longestIntegrationObjectItemCodeLength, integrationObjectItemCode.length(), impexBuilder);
        impexBuilder.append(HEADER_TAB_DELIMITER);
    }
    ...
    private int[] calculateAttributeColumnsLength(final Set<IntegrationObjectItemModel> items)
    {
        ...
        for(final IntegrationObjectItemModel item : items)
        {
            ...
            {
                final int attributeNameLength = attr.getAttributeName().length();
                longestAttribute = Math.max(longestAttribute, attributeNameLength);
                longestDescriptor = Math.max(longestDescriptor, (item.getType().getCode().length() + attributeNameLength));
    ...
    private void calculateLongestStrings(final Set<IntegrationObjectItemModel> items)
    {
        int longestItemCode = 0;
        int longestItemTypeCode = 0;
        int longestItemMatchType = 0;
        for(final IntegrationObjectItemModel item : items)
        {
            longestItemCode = Math.max(longestItemCode, item.getCode().length());
            longestItemTypeCode = Math.max(longestItemTypeCode, item.getType().getCode().length());
        ...
        longestIntegrationObjectItemCodeLength = longestItemCode;
        longestIntegrationObjectItemTypeCodeLength = longestItemTypeCode;
        longestItemMatchTypeStringLength = longestItemMatchType;
    }
}
