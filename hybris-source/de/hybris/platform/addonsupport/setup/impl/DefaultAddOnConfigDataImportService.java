...
public class DefaultAddOnConfigDataImportService implements AddOnConfigDataImportService
{
                    ...
                    final ImpexMacroParameterData macroParameters)
    {
        ...
        else if(AddonConfigDataImportType.SEARCH_SERVICES == importType)
        {
            return executeSearchServicesImport(extensionName, macroParameters);
        }
    ...
    protected boolean executeSearchServicesImport(final String extensionName, final ImpexMacroParameterData macroParameters)
    {
        final String path = String.format("/%s/import/searchservices/template/", extensionName);
        boolean imported = false;
        imported |= getSetupImpexService().importImpexFile(path + "searchservices.impex", macroParameters, false, false);
        imported |= getSetupImpexService().importImpexFile(path + "searchservicestrigger.impex", macroParameters, false, false);
        return imported;
    }
